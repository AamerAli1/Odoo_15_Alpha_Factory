...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        res = super(ReportPartnerLedger, self)._get_lines(options=options, line_id=line_id)
        company_currency = self.env.company.currency_id
        amount_currency_total = 0.0
        amount_list = []
        tmp_list = []
        for lines in res:
            if len(lines['columns']) == 10:
                amount = lines['columns'][8]['name'].split('SR')[0].strip()
                amount_list.append(float(amount.replace(',', '')))
                amount_currency_total = sum(x for x in amount_list)
            if type(lines['id']) is not int:
                tmp_list.append(lines['columns'])
            elif self._table == 'account_consolidation_trial_balance_report':
                pass
            # else:
            #     lines['columns'][3]['name'] = "{0:,.5f}".format(amount_currency_total) + ' ' + company_currency.symbol
        for i in tmp_list:
            i[3]['name'] = "{0:,.5f}".format(amount_currency_total) + ' ' + company_currency.symbol
        return res
