		...
		if res_config:
			for self_obj in self:
				...
				if res_config.tax_discount_policy == 'untax':
					if self_obj.discount_type == 'global':
						if self_obj.discount_method == 'fix':
								...
								for line in self_obj.invoice_line_ids:
									if line.tax_ids:
										if self_obj.amount_untaxed:
											final_discount = ((self_obj.discount_amt*line.price_subtotal)/self_obj.amount_untaxed)
											discount = (line.price_unit * line.quantity) - final_discount
											taxes = line.tax_ids.compute_all(discount, self_obj.currency_id, line.quantity,line.product_id,self.partner_id,is_refund=None,handle_price_include=None)
							...
							sums = 0.0
							before_amount_tax = self_obj.amount_tax
							self_obj.discount_amt = self_obj.amount_untaxed * (self_obj.discount_amount / 100) if self_obj.amount_untaxed > 0 else 0
							if self_obj.invoice_line_ids:
								for line in self_obj.invoice_line_ids:
									if line.tax_ids:
										if self_obj.amount_untaxed:
					...
					if self_obj.discount_type == 'global':
						if self_obj.discount_method == 'fix':
							...
							self_obj.discount_amt = (self_obj.amount_untaxed + self_obj.amount_tax) * (self_obj.discount_amount / 100.0)
					else:
						self_obj.discount_amt = 0.0
				...
				if self_obj.move_type == 'in_invoice' and not res_config.purchase_account_id:
					return
				if self_obj.move_type != 'entry' and (self_obj.discount_method in ('fix','per') and self_obj.discount_amount > 0 \
					or any(line.discount_method in ('fix','per') and line.discount_amount > 0 for line in self_obj.invoice_line_ids)):
		...
		for res in self:
			...
			if move_line and move_line.move_id.is_invoice():
				...
				if move_id.is_inbound():
					move_line_id = res.line_ids.filtered(lambda x : not x.name == 'Discount' and (x.debit > 0.0 and x.credit == 0.0))
